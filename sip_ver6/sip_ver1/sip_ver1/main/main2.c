...
void test_task(void *param)
{
    ...
    memset(tmp, 0, 512);
    exosip_init();
    ret = eXosip_call_build_initial_invite(excontext, &invite, dest_call, source_call, NULL, "This is a call for conversation");
    if (ret < 0) {
        printf("initial invite fail\n");
    }
    snprintf(tmp, 512, 
                      "v=0\r\n"
    ...
    ret = osip_message_set_body(invite, tmp, strlen(tmp));
    if (ret < 0) printf("set body fail\n");
    ret = osip_message_set_content_type(invite, "application/sdp");
    if (ret < 0) printf("set body fail\n");
    eXosip_lock(excontext);
    ret = eXosip_call_send_initial_invite(excontext, invite); //invite SIP INVITE message to send
    if (ret < 0) {
        printf("send initial fail\n");
    }
    eXosip_unlock(excontext);
    je = eXosip_event_wait(excontext, 5, 200);
    if (!je) {
        printf("event wait fail\n");
    }
    printf("event type: %d\n", je->type);
...
void app_main(void)
{
    ...
    esp_netif_init();
    wifi_sta_start_up();
    xTaskCreate(test_task, "test_task", 1024 * 4, NULL, 8, NULL);
}
#if 0
