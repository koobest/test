...
static int cb_snd_message(osip_transaction_t * tr, osip_message_t * sip,
...
int
_eXosip_snd_message(struct eXosip_t *excontext, osip_transaction_t * tr, osip_message_t * sip, char *host,
...
static void cb_xixt_kill_transaction(int type, osip_transaction_t * tr)
{
	...
	if (type == OSIP_NICT_KILL_TRANSACTION) {
		...
		if (MSG_IS_NOTIFY(tr->orig_request) && tr->last_response == NULL) {
			...
			je = _eXosip_event_init_for_notify(EXOSIP_NOTIFICATION_REQUESTFAILURE,
											  jn, jd, tr);
			_eXosip_report_event(excontext, je, NULL);
		...
		if (js != NULL && MSG_IS_SUBSCRIBE(tr->orig_request)
			...
			je = _eXosip_event_init_for_subscribe
...
static void cb_rcvrequest(int type, osip_transaction_t * tr, osip_message_t * sip)
{
	...
	if (jc != NULL) {
		...
		if (MSG_IS_SUBSCRIBE(sip)) {
			eXosip_event_t *je;
			je = _eXosip_event_init_for_notify(EXOSIP_IN_SUBSCRIPTION_NEW, jn, jd,
											  tr);
			_eXosip_report_event(excontext, je, NULL);
			return;
		}
...
static void cb_rcv1xx(int type, osip_transaction_t * tr, osip_message_t * sip)
{
	...
	if (MSG_IS_RESPONSE_FOR(sip, "SUBSCRIBE") && MSG_TEST_CODE(sip, 100)) {
		eXosip_event_t *je;
		je = _eXosip_event_init_for_subscribe(EXOSIP_SUBSCRIPTION_PROCEEDING,
												js, jd, tr);
...
static void cb_rcv2xx_4subscribe(osip_transaction_t * tr, osip_message_t * sip)
{
...

	{
		...
		je = _eXosip_event_init_for_subscribe(EXOSIP_SUBSCRIPTION_ANSWERED, js, jd,
											 tr);
		_eXosip_report_event(excontext, je, sip);
	}
...
static void cb_rcv2xx(int type, osip_transaction_t * tr, osip_message_t * sip)
{
	...
	if (jn != NULL) {
		if (MSG_IS_RESPONSE_FOR(sip, "NOTIFY") && jn != NULL) {
			...
			je = _eXosip_event_init_for_notify(EXOSIP_NOTIFICATION_ANSWERED, jn, jd,
											  tr);
			_eXosip_report_event(excontext, je, sip);
...
static void cb_rcv3xx(int type, osip_transaction_t * tr, osip_message_t * sip)
{
	...
	if (MSG_IS_RESPONSE_FOR(sip, "INVITE")) {
		...
		je = _eXosip_event_init_for_notify(EXOSIP_NOTIFICATION_REDIRECTED, jn, jd,
										  tr);
		_eXosip_report_event(excontext, je, sip);
	} else if (MSG_IS_RESPONSE_FOR(sip, "SUBSCRIBE")) {
		eXosip_event_t *je;

		je = _eXosip_event_init_for_subscribe(EXOSIP_SUBSCRIPTION_REDIRECTED, js,
											 jd, tr);
		_eXosip_report_event(excontext, je, sip);
	} else if (jc != NULL) {
...
static void cb_rcv4xx(int type, osip_transaction_t * tr, osip_message_t * sip)
{
	...
	if (MSG_IS_RESPONSE_FOR(sip, "INVITE")) {
		...
		je = _eXosip_event_init_for_subscribe(EXOSIP_SUBSCRIPTION_REQUESTFAILURE,
											 js, jd, tr);
		_eXosip_report_event(excontext, je, sip);
	} else if (jc != NULL) {
		_eXosip_report_call_event(excontext, EXOSIP_CALL_MESSAGE_REQUESTFAILURE, jc, jd, tr);
...
static void cb_rcv5xx(int type, osip_transaction_t * tr, osip_message_t * sip)
{
	...
	if (MSG_IS_RESPONSE_FOR(sip, "INVITE")) {
		...
		je = _eXosip_event_init_for_notify(EXOSIP_NOTIFICATION_SERVERFAILURE,
										  jn, jd, tr);
		_eXosip_report_event(excontext, je, sip);
	} else if (MSG_IS_RESPONSE_FOR(sip, "SUBSCRIBE")) {
		eXosip_event_t *je;

		je = _eXosip_event_init_for_subscribe(EXOSIP_SUBSCRIPTION_SERVERFAILURE,
											 js, jd, tr);
		_eXosip_report_event(excontext, je, sip);
	} else if (jc != NULL) {
...
static void cb_rcv6xx(int type, osip_transaction_t * tr, osip_message_t * sip)
{
	...
	if (MSG_IS_RESPONSE_FOR(sip, "INVITE")) {
		...
		je = _eXosip_event_init_for_notify(EXOSIP_NOTIFICATION_GLOBALFAILURE,
										  jn, jd, tr);
		_eXosip_report_event(excontext, je, sip);
	} else if (MSG_IS_RESPONSE_FOR(sip, "SUBSCRIBE")) {
		eXosip_event_t *je;

		je = _eXosip_event_init_for_subscribe(EXOSIP_SUBSCRIPTION_GLOBALFAILURE,
											 js, jd, tr);
		_eXosip_report_event(excontext, je, sip);
	} else if (jc != NULL) {
