...
void eXosip_quit(struct eXosip_t *excontext)
{
	...
	memset(excontext, 0, sizeof(struct eXosip_t));
				   ...
				   int secure)
{
	...
	if (excontext->j_thread == NULL) {
        DEBUG();
		excontext->j_thread = (void *) osip_thread_create(1024*10, _eXosip_thread, excontext);
		DEBUG();
		if (excontext->j_thread == NULL) {
			DEBUG();
			OSIP_TRACE(osip_trace
...
{
	struct eXosip_t *ptr = (struct eXosip_t *)osip_malloc(sizeof(struct eXosip_t));
	if (ptr)
		memset(ptr , 0, sizeof(struct eXosip_t));
	return ptr;
}
...
int eXosip_execute(struct eXosip_t *excontext)
{
	...
	printf("get tout %d  %d\n",(int)lower_tv.tv_sec, (int)lower_tv.tv_usec);
		DEB();
