	...
	if (reserved->socket_tab[pos].ssl_type == 2) {
		if (reserved->socket_tab[pos].ssl_conn != NULL) {
			...
			i = _eXosip_get_addrinfo(NULL, &addrinfo,
									...
									reserved->socket_tab[pos].remote_port, IPPROTO_UDP);
			if (i != 0) {
				return -1;
			}
...
static int dtls_tl_open(struct eXosip_t *excontext)
{
	...
	snprintf(reserved->dtls_firewall_port, sizeof(reserved->dtls_firewall_port), "%i",
			 eXtl_dtls.proto_port);
	return OSIP_SUCCESS;
}
...
static int dtls_tl_read_message(struct eXosip_t *excontext, fd_set * osip_fdset, fd_set * osip_wrset)
{
	...
	if (FD_ISSET(reserved->dtls_socket, osip_fdset)) {
		...
		if (enc_buf_len > 5) {
			...
			for (pos = 0; pos < EXOSIP_MAX_SOCKETS; pos++) {
				if (reserved->socket_tab[pos].ssl_conn != NULL) {
					if (reserved->socket_tab[pos].remote_port == recvport &&
						(strcmp(reserved->socket_tab[pos].remote_ip, src6host) == 0)) {
			...
			if (reserved->socket_tab[pos].ssl_conn == NULL) {
				...
				SSL_set_options(reserved->socket_tab[pos].ssl_conn,
								SSL_OP_NO_QUERY_MTU);
			...
			i = SSL_read(reserved->socket_tab[pos].ssl_conn, dec_buf,
			...
			if (i > 5) {
				...
				if (err == SSL_ERROR_SYSCALL) {
					...
					memset(&(reserved->socket_tab[pos]), 0,
						   sizeof(reserved->socket_tab[pos]));
				}
			} else {
				OSIP_TRACE(osip_trace(__FILE__, __LINE__, OSIP_INFO1, NULL,
	...
	if (reserved->dtls_firewall_ip != '\0') {
		...
		while (!osip_list_eol(&req->contacts, pos)) {
			...
			if (co != NULL && co->url != NULL && co->url->host != NULL
				&& 0 == osip_strcasecmp(co->url->host, reserved->dtls_firewall_ip)) {
				if (co->url->port == NULL &&
					0 != osip_strcasecmp(reserved->dtls_firewall_port, "5061")) {
					co->url->port = osip_strdup(reserved->dtls_firewall_port);
				} else if (co->url->port != NULL &&
						   0 != osip_strcasecmp(reserved->dtls_firewall_port, co->url->port))
				{
					 ...
					 int port, int out_socket)
{
	...
	for (pos = 0; pos < EXOSIP_MAX_SOCKETS; pos++) {
		if (reserved->socket_tab[pos].ssl_conn != NULL
			&& reserved->socket_tab[pos].ssl_type == EXOSIP_AS_A_SERVER) {
			if (reserved->socket_tab[pos].remote_port == port &&
				(strcmp(reserved->socket_tab[pos].remote_ip, ipbuf) == 0)) {
				BIO *rbio;
				_dtls_stream_used = &reserved->socket_tab[pos];
				rbio = BIO_new_dgram(reserved->dtls_socket, BIO_NOCLOSE);
				BIO_ctrl(rbio, BIO_CTRL_DGRAM_SET_PEER, 0, (char *)&addr);
	...
	if (_dtls_stream_used == NULL) {
		for (pos = 0; pos < EXOSIP_MAX_SOCKETS; pos++) {
			if (reserved->socket_tab[pos].ssl_conn != NULL
				&& reserved->socket_tab[pos].ssl_type == EXOSIP_AS_A_CLIENT) {
				if (reserved->socket_tab[pos].remote_port == port &&
					(strcmp(reserved->socket_tab[pos].remote_ip, ipbuf) == 0)) {
					BIO *rbio;
					_dtls_stream_used = &reserved->socket_tab[pos];
	...
	if (reserved->socket_tab[pos].ssl_conn == NULL) {
		...
		osip_strncpy(reserved->socket_tab[pos].remote_ip, ipbuf,
					 sizeof(reserved->socket_tab[pos].remote_ip) - 1);
...
static int dtls_tl_keepalive(struct eXosip_t *excontext)
{
	...
	for (jr = excontext->j_reg; jr != NULL; jr = jr->next) {
		if (jr->len > 0) {
			if (sendto(reserved->dtls_socket, (const void *) buf, 4, 0,
