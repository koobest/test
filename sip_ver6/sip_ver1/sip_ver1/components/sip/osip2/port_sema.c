...
#if !defined(_FreeRTOS_) && !defined(__VXWORKS_OS__) && !defined(__PSOS__) && !defined(WIN32) && !defined(_WIN32_WCE) && !defined(HAVE_PTHREAD_WIN32) && !defined(HAVE_PTHREAD) && !defined(HAVE_PTH_PTHREAD_H) && !defined(__rtems__)
...
{
    struct osip_mutex *mutex = (struct osip_mutex *) osip_malloc(sizeof(struct osip_mutex));
    if (!mutex) return NULL;
    mutex->mutex = xSemaphoreCreateMutex();
    return (struct osip_mutex *)mutex;
}
...
int osip_mutex_unlock(struct osip_mutex *mut)
{
    if (mut == NULL)
    return OSIP_BADPARAMETER;
    xSemaphoreGive(mut->mutex);
    return 0;
}

struct osip_sem *osip_sem_init(unsigned int value)
{
    struct osip_sem *sem = (struct osip_sem *) osip_malloc(sizeof(struct osip_sem));
    if (!sem) return NULL;
...
int osip_sem_wait(struct osip_sem *sem)
{
    if (sem == NULL)
    return OSIP_BADPARAMETER;
    xSemaphoreTake(sem->sem, (TickType_t)portMAX_DELAY);
    return 0;
}
...
int osip_sem_trywait(struct osip_sem *sem)
{
    if (sem == NULL)
    return OSIP_BADPARAMETER;
    if(xSemaphoreTake(sem->sem, (TickType_t)0) != pdTRUE)
      return OSIP_UNDEFINED_ERROR;
    return 0;
}
