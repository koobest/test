...
struct osip_thread *osip_thread_create(int stacksize, void *(*func)(void *), void *arg) {
  ...
  thread->tid = NULL;
  int status = xTaskCreate((void *)func, "SIP", stacksize, arg, 10, &(thread->tid));
  if ((status != pdPASS) || (thread->tid == NULL)) {
    printf("task create fail %d  %p  %d\n", status, thread->tid, stacksize);
    osip_free(thread);
    thread = NULL;
  } else {  
...
#if 0
  int status = xTaskCreate(test_task, "test_task", 1024*4, NULL, 10, &(thread->tid));
  if ((status != pdPASS) || (thread->tid == NULL)) {
    printf("task create fail %d  %p  %d\n", status, thread->tid, stacksize);
    osip_free(thread);
    thread = NULL;
...
int osip_thread_set_priority(struct osip_thread *_thread, int priority) {
  ...
  vTaskPrioritySet(thread->tid, (UBaseType_t)priority);
  return OSIP_SUCCESS;
}
...
int osip_thread_join(struct osip_thread *_thread) {
  ...
  return OSIP_BADPARAMETER;
  while (1) {
      eTaskState stat = eTaskGetState((((osip_thread_t *)_thread)->tid));
      if( stat == eDeleted || stat == eInvalid) {
          break;
      }
      vTaskDelay(1000/portTICK_PERIOD_MS);
  }
